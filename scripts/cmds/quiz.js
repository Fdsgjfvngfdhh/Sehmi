...
module.exports = {
  ...
  onStart: async function ({ message, event, usersData, commandName, getLang, args, api }) {
    ...
    if (!['english', 'math', 'physics', 'filipino', 'biology', 'chemistry', 'history', 'philosophy', 'random', 'torf', 'science', 'flag', 'anime'].includes(category)) {
    ...
    try {
      let response;
      if (category === 'torf') {
        response = await axios.get(`https://aryanchauhanapi.onrender.com/api/quiz?category=torf`);
        ...
        response = await axios.get(`https://aryanchauhanapi.onrender.com/api/quiz?category=anime`);
        const Qdata = response.data;

        if (!Qdata || !Qdata.image || !Qdata.fullName || !Qdata.traits) {
          return;
        }

        const imageUrl = Qdata.image;
        const characterName = Qdata.fullName;
const hint = Qdata.traits;
      
       message.reply({
          attachment: await global.utils.getStreamFromURL(imageUrl),
         body: `📝| 𝗛𝗶𝗻𝘁: ${hint}\n\nPlease reply with the character's full name in small latter`
        }, async (err, info) => {
        ...
        response = await axios.get(`https://aryanchauhanapi.onrender.com/api/quiz?category=flag`);
        const Qdata = response.data;

        if (!Qdata || !Qdata.link || !Qdata.country) {
          return;
        }

        const imageUrl = Qdata.link;
        const countryName = Qdata.country;

        message.reply({
          attachment: await global.utils.getStreamFromURL(imageUrl),
          body: `Please reply with the correct Country's Name.`
        }, async (err, info) => {
          global.GoatBot.onReply.set(info.messageID, {
            commandName,
            messageID: info.messageID,
        ...
        response = await axios.get(`https://aryanchauhanapi.onrender.com/api/quiz?category=${category}`);
        const Qdata = response.data;
  ...
  onReaction: async function ({ message, event, Reaction, api, usersData }) {
    try {
      ...
      if (isCorrect) {
        ...
        api.sendMessage(`🎉 Congratulations ${userName}! You provided the correct answer and won ${reward} $.`, event.threadID, event.messageID);
      } else {
